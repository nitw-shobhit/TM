package com.tm.core.query;

public class QueryConstants extends ParamConstants {

	public static final String GET_PROJECT_BY_USER = "getProjectsByUser";
	public static final String GET_USER_BY_USER_ID = "getUserByUserId";
	public static final String GET_USERPROJ_BY_PROJ_ID = "getUserProjectByProjectId";
	public static final String GET_USERS_BY_QUERY = "getUsersByQuery";
	public static final String GET_MODULES_BY_PROJ_ID = "getModulesByProjectId";
}
